...
# Пороги детектора и режим вывода.
#
# `Global.max_side_len` — единственный рычаг разрешения, который действительно
# работает: он ужимает страницу целиком до детектора. Значение равно
# библиотечному и закреплено явно, потому что от него зависит качество, а
# умолчание чужой библиотеки менять некому.
#
# `Det.limit_side_len` здесь намеренно нет. При `Det.limit_type = "max"`
# RapidOCR его не читает вовсе: `TextDetector.get_preprocess` в этой ветке сам
# выбирает 960, 1500 или 2000 по размеру картинки. Настроенное значение
# создавало видимость управления масштабом, которой не было.
#
# Поднимать разрешение пробовали на корпусе: 2048, 2560, 3072 и 3520 при
# `limit_type = "min"`. Строк находится больше, но CER не улучшается, чистый
# скан остаётся около 0.29-0.35, зашумлённый портится вдвое сильнее, а прогон
# замедляется в 1.6-2.5 раза. Причина потерь не в масштабе.
