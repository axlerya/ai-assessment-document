...
def test_domain_package_has_no_third_party_imports() -> None:
    ...
    for module_path in sorted(domain_root.rglob("*.py")):
        ...
        for imported in _iter_imported_modules(module_path):
            ...
            if top_level == "ai_worker":
                if not imported.startswith("ai_worker.domain"):
...
def test_service_never_imports_the_other_service() -> None:
    # Чанки читаются из общей базы (ADR-0001), а не импортом чужого пакета:
    # импорт связал бы сборку и выкатку двух сервисов намертво.
    offenders = [
        f"{module_path.relative_to(SERVICE_ROOT).as_posix()}: {imported}"
        for module_path in sorted(PACKAGE_ROOT.rglob("*.py"))
        for imported in _iter_imported_modules(module_path)
        if imported.split(".", maxsplit=1)[0] == "document_worker"
    ]

    assert not offenders, "ai-worker импортирует пакет document-worker: " + ", ".join(
        offenders
    )
